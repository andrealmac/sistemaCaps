#importar biblioteca que trabalha com binario
import pickle

def salvarUsuario(lista):
    arquivo = open('prontuario.bin', 'wb')
    pickle.dump(lista, arquivo)#para onde ira salvar
    arquivo.close()

def carregarUsuario():
    lista = []
    try:
        arquivo = open('prontuario.bin', 'rb')
        lista = pickle.load(arquivo)#da onde vai carregar e para onde vai retornar    
        arquivo.close()
